...
uint256 hashGenesisBlock("0xd5bfcf54a4597910017351ceebccd087c97448f461d1d509370139faf7eb68d3");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(504365644); // Priv: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Priv Signed Message:\n";
...
{
    // Priv: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Priv: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 86400; // Priv: 3.5 days
static const int64 nTargetSpacing = 150; // Priv: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Priv: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Priv: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Priv: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0313022e2e756ac6c88bcfb2931fd5506f51bf01500954c34e71b775383733b3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Priv - Blockchain-tailored authentication platform";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5000000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0b444db643cd607c10d5914d1b5229a150e6490c49914faf03679aa06246b05585ca55c70a436063760bf00784d77c9dadb5cafc1a564d55ae97977bf79c0913ba") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1494972448;
        block.nBits    = 504365644;
        block.nNonce   = 4204469567;

        if (fTestNet)
        {
            block.nTime    = 1494972448;
            block.nNonce   = 3960871492;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x8a7bf365058cc5cd9b65a023fe220298f7a56ff947f58f3c809708e4383f48a8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Priv: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PrivMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PrivMiner : ProcessBlock, block not accepted");
    }
